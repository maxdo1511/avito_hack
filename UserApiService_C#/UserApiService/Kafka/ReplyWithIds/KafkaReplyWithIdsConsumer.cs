...
public class KafkaReplyWithIdsConsumer : IKafkaConsumer
{
    ...
    private IConsumer<Guid, ReplyWithIds> _consumer; 
    ...
    public KafkaReplyWithIdsConsumer(string topicName, bool EnableAutoCommit, int AutoCommitIntervalMs, bool EnableAutoOffsetStore, AutoOffsetReset autoOffsetReset, string groupId)
    {
        ...
        _consumer = new ConsumerBuilder<Guid, ReplyWithIds>(_config)
            .SetValueDeserializer(new JsonDeserializer<ReplyWithIds>().AsSyncOverAsync()).Build();
    ...
    public async Task Consume()
    {
        try
        {
            while (true)
            {
                ConsumeResult<Guid, ReplyWithIds> result = _consumer.Consume(5000);
    ...
    public async Task<IReply> ConsumeReply(Guid MessageKey)
    {
        try
        {
            while (true)
